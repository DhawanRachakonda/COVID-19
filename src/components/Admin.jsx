...
function InfectedAreas({infectedList}) {
    ...
    return (
        <CardColumns style={{marginTop: "2.5rem"}}>
            {infectedList.map((infectedPlace, index) => <Card key={index}>
                <Card.Body>
...
function InfectedPlaceForm() {
    ...
    const addInfectedPlace = (e) => {
        ...
        if(selectedDate && place) {
            setInfectedList([...infectedList, {place, data: selectedDate.toDateString()}]);
        ...
        <React.Fragment>
            <Form>
                <Form.Group controlId="formBasicEmail">
                    ...
                    <PlacesAutocomplete value={place} onChange={handleChange} onSelect={handleSelect}>
                        {renderFunc}
                    </PlacesAutocomplete>
                </Form.Group>
            
                <Form.Group controlId="formBasicPassword">
                    <Form.Label>Infected Date</Form.Label><br/>
                    <DatePicker
...
function UploadFromGoogle() {
    return <div style={{marginTop: "2.5rem"}}><a href="https://takeout.google.com/">Upload Google Data</a></div>
}

export default function AdminHomePage() {
    return (
        <Tabs defaultActiveKey="profile" id="uncontrolled-tab-example" style={{marginTop: "2.5rem"}}>
